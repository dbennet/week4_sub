namespace :ptourist do
  ...
  def create_thing thing, organizer, members, images, service
    ...
    if service
    service.each do |ser|
      puts "building service for #{thing.name}, #{ser[:caption]}, by #{organizer.name}"
      sg=BusinessService.create(:creator_id=>organizer.id,:caption=>ser[:caption])
      organizer.add_role(Role::ORGANIZER, sg).save
      ThingService.new(:thing=>thing, :business_service=>sg,
      :creator_id=>organizer.id)
      .tap {|ti| ti.priority=sg[:priority] if sg[:priority]}.save!
    end
    end
    ...
    # if service
    #   service.each do |ser|
    #     ser['thing_id'] = thing.id
    #     ser=BusinessService.create!(ser)
    #     organizer.add_role(Role::ORGANIZER, ser).save
    #     m=members.map { |member|
    #       unless (member.id==organizer.id || member.id==mike_user.id)
    #         member.add_role(Role::MEMBER, ser).save
    #         member
    #       end
    #     }.select {|r| r}
    #   end
    ...
    puts organizer.name
    service=[{:caption=>"ATM"}]
    create_thing thing, organizer, members, images, service
    ...
    service=[{:caption=>"ATM", :creator_id=>organizer.id },{:caption=>"Parking", :creator_id=>organizer.id },{:caption=>"Bathrooms", :creator_id=>organizer.id }]
    create_thing thing, organizer, members, images, service
    ...
    service=[{:caption=>"ATM", :creator_id=>organizer.id },{:caption=>"Parking", :creator_id=>organizer.id },{:caption=>"24Hr Reception", :creator_id=>organizer.id },{:caption=>"Bar", :creator_id=>organizer.id }]
    create_thing thing, organizer, members, images, service
    ...
    service=[{:caption=>"Breakfast Buffet", :creator_id=>organizer.id },{:caption=>"ATM", :creator_id=>organizer.id },{:caption=>"Parking", :creator_id=>organizer.id },{:caption=>"24Hr Reception", :creator_id=>organizer.id },{:caption=>"Bar", :creator_id=>organizer.id }]
    ...
    puts "#{Thing.count} things created and image created #{ThingImage.count("distinct thing_id")} and #{BusinessService.count} with seruiries"
